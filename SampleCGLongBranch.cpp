//===-- SampleCGLongBranch.cpp - Emit long branches ---------------------------===//
//
...
#define DEBUG_TYPE "samplecg-long-branch"

STATISTIC(LongBranches, "Number of long branches.");

static cl::opt<bool> ForceLongBranch(
  "force-samplecg-long-branch",
  cl::init(false),
  cl::desc("SAMPLECG: Expand all branches to long format."),
  cl::Hidden);
...
namespace {
  ...
  class SampleCGLongBranch : public MachineFunctionPass {

  public:
    static char ID;
    SampleCGLongBranch(TargetMachine &tm)
        : MachineFunctionPass(ID), TM(tm), IsPIC(TM.isPositionIndependent()),
          ABI(static_cast<const SampleCGTargetMachine &>(TM).getABI()) {}

    StringRef getPassName() const override {
      return "SampleCG Long Branch";
    }
  ...
  char SampleCGLongBranch::ID = 0;
} // end of anonymous namespace

/// createSampleCGLongBranchPass - Returns a pass that converts branches to long
/// branches.
FunctionPass *llvm::createSampleCGLongBranchPass(SampleCGTargetMachine &tm) {
  return new SampleCGLongBranch(tm);
}
...
void SampleCGLongBranch::splitMBB(MachineBasicBlock *MBB) {
...
// Fill MBBInfos.
void SampleCGLongBranch::initMBBInfo() {
	const SampleCGSubtarget &Subtarget =
      static_cast<const SampleCGSubtarget &>(MF->getSubtarget());
  const SampleCGInstrInfo *TII =
      static_cast<const SampleCGInstrInfo *>(Subtarget.getInstrInfo());
...
int64_t SampleCGLongBranch::computeOffset(const MachineInstr *Br) {
  int64_t Offset = 0;
...
void SampleCGLongBranch::replaceBranch(MachineBasicBlock &MBB, Iter Br,
                                   const DebugLoc &DL,
                                   MachineBasicBlock *MBBOpnd) {
  const SampleCGInstrInfo *TII = static_cast<const SampleCGInstrInfo *>(
...
// are present.
void SampleCGLongBranch::expandToLongBranch(MBBInfo &I) {
  ...
  if (IsPIC) {
    ...
    BuildMI(*LongBrMBB, Pos, DL, TII->get(SampleCG::ADDiu), SampleCG::SP)
      .addReg(SampleCG::SP).addImm(-8);
    BuildMI(*LongBrMBB, Pos, DL, TII->get(SampleCG::ST)).addReg(SampleCG::LR)
      .addReg(SampleCG::SP).addImm(0);
    ...
    BuildMI(*LongBrMBB, Pos, DL, TII->get(SampleCG::LONG_BRANCH_LUi), SampleCG::AT)
      .addMBB(TgtMBB).addMBB(BalTgtMBB);
    BuildMI(*LongBrMBB, Pos, DL, TII->get(SampleCG::LONG_BRANCH_ADDiu), SampleCG::AT)
      .addReg(SampleCG::AT).addMBB(TgtMBB).addMBB(BalTgtMBB);
    MIBundleBuilder(*LongBrMBB, Pos)
        .append(BuildMI(*MF, DL, TII->get(BalOp)).addMBB(BalTgtMBB));

    Pos = BalTgtMBB->begin();

    BuildMI(*BalTgtMBB, Pos, DL, TII->get(SampleCG::ADDu), SampleCG::AT)
      .addReg(SampleCG::LR).addReg(SampleCG::AT);
    BuildMI(*BalTgtMBB, Pos, DL, TII->get(SampleCG::LD), SampleCG::LR)
      .addReg(SampleCG::SP).addImm(0);
    BuildMI(*BalTgtMBB, Pos, DL, TII->get(SampleCG::ADDiu), SampleCG::SP)
      .addReg(SampleCG::SP).addImm(8);

    MIBundleBuilder(*BalTgtMBB, Pos)
      .append(BuildMI(*MF, DL, TII->get(SampleCG::JR)).addReg(SampleCG::AT))
      .append(BuildMI(*MF, DL, TII->get(SampleCG::NOP)));
    ...
    MIBundleBuilder(*LongBrMBB, Pos)
      .append(BuildMI(*MF, DL, TII->get(SampleCG::JMP)).addMBB(TgtMBB))
      .append(BuildMI(*MF, DL, TII->get(SampleCG::NOP)));
...
static void emitGPDisp(MachineFunction &F, const SampleCGInstrInfo *TII) {
  MachineBasicBlock &MBB = F.front();
  MachineBasicBlock::iterator I = MBB.begin();
  DebugLoc DL = MBB.findDebugLoc(MBB.begin());
  BuildMI(MBB, I, DL, TII->get(SampleCG::LUi), SampleCG::V0)
    .addExternalSymbol("_gp_disp", SampleCGII::MO_ABS_HI);
  BuildMI(MBB, I, DL, TII->get(SampleCG::ADDiu), SampleCG::V0)
    .addReg(SampleCG::V0).addExternalSymbol("_gp_disp", SampleCGII::MO_ABS_LO);
  MBB.removeLiveIn(SampleCG::V0);
}

bool SampleCGLongBranch::runOnMachineFunction(MachineFunction &F) {
  const SampleCGSubtarget &STI =
      static_cast<const SampleCGSubtarget &>(F.getSubtarget());
  const SampleCGInstrInfo *TII =
      static_cast<const SampleCGInstrInfo *>(STI.getInstrInfo());
  LongBranchSeqSize =
      !IsPIC ? 2 : 10;

  if (!STI.enableLongBranchPass())
    return false;
  if (IsPIC && static_cast<const SampleCGTargetMachine &>(TM).getABI().IsO32() &&
      F.getInfo<SampleCGFunctionInfo>()->globalBaseRegSet())
    emitGPDisp(F, TII);
